...
from ..layers.common import ConvNormAct, SubMConvNormAct
from ...utils.debug import debug_hook
...
class BEVConvHead(nn.Module):
    def __init__(
        ...
        self._get_layers(dict_input, shared_out_c, num_classes, self.dense_input)

    def _get_layers(self, dict_input, shared_out_c, num_classes, dense_input):
        # Unpack
        ...
        if dense_input:
            ...
            convnormact_conv_cn = nn.Sequential(
                ConvNormAct(shared_out_c, shared_out_c, 3, 1, False, norm, activ),
                nn.Conv2d(shared_out_c, out_channels=num_classes, kernel_size=1, padding=0),
            )
            if with_hdmap:
            ...
            convnormact_conv_cn = spconv.SparseSequential(
                SubMConvNormAct(shared_out_c, shared_out_c, 3, 1, False, norm, activ),
                spconv.SubMConv2d(
                    shared_out_c, out_channels=num_classes, kernel_size=1, padding=0, algo=algo
                ),
            )
            if with_hdmap:
