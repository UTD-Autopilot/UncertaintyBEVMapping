...
class EvidentialTopK(Model):
    ...
    def loss_ood(self, alpha, y, ood, mapped_uncertainty, mapped_labels, K=40):
    ...
    def train_step_ood(self, images, intrinsics, extrinsics, labels, ood, mapped_uncertainty, mapped_labels):
        self.opt.zero_grad(set_to_none=True)

        outs = self(images, intrinsics, extrinsics)
        loss, oodl = self.loss_ood(outs, labels.to(self.device), ood, mapped_uncertainty.to(self.device), mapped_labels)
        loss.backward()
