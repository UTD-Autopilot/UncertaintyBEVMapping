...
class ExtRandomScale(object):
    ...
    def __call__(self, img, *lbls):
        ...
        """
        assert img.size == lbls[0].size
        scale = random.uniform(self.scale_range[0], self.scale_range[1])
        target_size = ( int(img.size[1]*scale), int(img.size[0]*scale) )
        return tuple([F.resize(img, target_size, self.interpolation)]) + tuple(F.resize(lbl, target_size, Image.NEAREST) for lbl in lbls)
...
class ExtPad(object):
    ...
    def __call__(self, *imgs):
        ...
        return (F.pad(img, (pw//2, pw-pw//2, ph//2, ph-ph//2)) for img in imgs)
...
class ExtToTensor(object):
    ...
    def __call__(self, pic, *lbls):
        ...
        """
        if self.normalize:
            return tuple([F.to_tensor(pic)]) + tuple(torch.from_numpy(np.array(lbl,dtype=self.target_type)) for lbl in lbls)
        else:
            return tuple([torch.from_numpy( np.array( pic, dtype=np.float32).transpose(2, 0, 1) )]) + tuple(torch.from_numpy(np.array(lbl,dtype=self.target_type)) for lbl in lbls)
...
class ExtNormalize(object):
    ...
    def __call__(self, tensor, *lbls):
        ...
        """
        return tuple([F.normalize(tensor, self.mean, self.std)]) + tuple((lbl for lbl in lbls))
    ...
    def __call__(self, *imgs):
        ...
        if self.padding > 0:
            for i in range(len(imgs)):
                imgs[i] = F.pad(imgs[i], self.padding)

        # pad the width if needed
        if self.pad_if_needed and imgs[0].size[0] < self.size[1]:
            for i in range(len(imgs)):
                imgs[i] = F.pad(imgs[i], padding=int((1 + self.size[1] - imgs[0].size[0]) / 2))

        # pad the height if needed
        if self.pad_if_needed and imgs[0].size[1] < self.size[0]:
            for i in range(len(imgs)):
                imgs[i] = F.pad(imgs[i], padding=int((1 + self.size[0] - imgs[0].size[1]) / 2))
...
class ExtResize(object):
    ...
    def __call__(self, img, *lbls):
        ...
        Returns:
            PIL Image: Rescaled image.
        """
        return tuple([F.resize(img, self.size, self.interpolation)]) + tuple(F.resize(lbl, self.size, Image.NEAREST) for lbl in lbls)
...
class ExtColorJitter(object):
    ...
    def __call__(self, img, *lbls):
        ...
        return tuple([transform(img)]) + tuple((lbl for lbl in lbls))
